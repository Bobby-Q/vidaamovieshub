(function () {
  ...
  var state = {
    ...
    likes: {}, ranked: null, likeCounts: {}, similar: []
  };
  ...
  function moveFocus(direction) {
    ...
    if (direction === 'right' || direction === 'left') {
      ...
      } else if (section.type === 'row' || section.type === 'detail-recommended' || section.type === 'detail-similar') {
      ...
      if (section.type === 'nav') {
        ...
        else { targetType = 'detail-sources'; targetIndex = 13; }
      } else if (section.type === 'detail-sources') {
  ...
  function renderHome() {
    ...
    hero.innerHTML = '<div class="hero-content"><div class="meta-line">' + esc((featured.genres || []).join(' • ')) + ' • ' + esc(runtime(featured)) + ' • ' + esc(rating(featured)) + ' • ' + esc(featured.year || '') + '</div><h1>' + esc(featured.title) + '</h1><p>' + esc(featured.overview) + '</p><div class="button-row netflix-row"><div class="btn-play" data-focusable="true" data-action="play-featured" data-id="' + esc(featured.id) + '">▶ Play</div><div class="btn-secondary" data-focusable="true" data-action="details" data-id="' + esc(featured.id) + '">ⓘ More Info</div><div class="btn-secondary" data-focusable="true" data-action="next-hero">→ Next</div></div></div>';
  ...
  function renderDetails(id) {
    ...
    return Promise.all([api('/api/media/' + id), api('/api/playback/' + id), api('/api/similar/' + id)]).then(function (results) {
      ...
      var wrap = el('section', 'detail-hero'); wrap.style.setProperty('--detail-bg', 'url(' + esc(backdrop(item)) + ')');
      var trailerHtml = '';
      var muteHtml = '';
      if (item.trailer && item.trailer.key) {
      ...
      wrap.innerHTML = trailerHtml + '<div class="detail-fade"></div>' + muteHtml + '<div class="detail-copy"><h1>' + esc(item.title) + '</h1><div class="badge-row"><span>' + esc(item.year || '') + '</span><span>HDR</span><span>4K</span><span class="rating-badge">' + esc(rating(item)) + '</span><span>' + esc(runtime(item)) + '</span></div><div class="genre-row">' + (item.genres || []).map(function (g) { return '<span>' + esc(g) + '</span>'; }).join('') + '</div><div class="button-row netflix-row"><div class="btn-play" data-focusable="true" data-action="play">▶ Play</div><div class="btn-secondary" data-focusable="true" data-action="trailer">▶ Trailer</div><div class="btn-circle" data-focusable="true" data-action="like">♥<span class="like-count">' + likeCount + '</span></div><div class="btn-circle" data-focusable="true" data-action="favorite">' + (isFavorite(item.id) ? '✓' : '+') + '</div></div><h2>Overview</h2><p class="overview">' + esc(item.overview) + '</p><h2>Cast</h2><div class="cast-row">' + (item.cast || []).map(function (c) { return '<div class="cast-chip"><span style="background-image:url(' + esc(wsrv(c.photo || '')) + ')"></span>' + esc(c.name) + '</div>'; }).join('') + '</div><h2>Recommended</h2><div class="recommended-wrap"><div class="card-row recommended"></div></div><h2>Similar</h2><div class="similar-wrap"><div class="card-row similar"></div></div><h2>Sources</h2><div class="source-list"></div></div>';
      ...
      var similar = screen.querySelector('.similar'); state.similar.slice(0, 8).forEach(function (s) { similar.appendChild(renderCard(s)); });
      ...
      if (state.focusMemory.details === undefined) {
  ...
  function activate(node) {
    ...
    if (action === 'trailer') {
      if (state.current && state.current.trailer && state.current.trailer.key) {
        window.open('https://www.youtube.com/watch?v=' + state.current.trailer.key, '_blank');
      }
    }
    if (action === 'next-hero') { nextHeroSlide(); }
